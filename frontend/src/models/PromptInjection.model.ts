...
export interface IPromptInjection extends Document {
...
const PromptInjectionSchema = new Schema<IPromptInjection>(
  {
  ...
  {
    collection: "promptinjections",
  },
);

// Check if model exists before defining to prevent overwrite error in dev mode
const PromptInjection: Model<IPromptInjection> =
  mongoose.models.PromptInjection ||
  mongoose.model<IPromptInjection>("PromptInjection", PromptInjectionSchema);
