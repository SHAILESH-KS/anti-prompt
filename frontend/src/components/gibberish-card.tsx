...
import { IGibberishEntity, IScannerInfo } from "@/src/models/gibberish.model";
...
interface GibberishCardProps {
  data: GibberishResult;
}

export function GibberishCard({ data }: GibberishCardProps) {
  const {
    sanitized_prompt,
    is_valid,
    risk_score,
      ...
      <CardContent className="space-y-4 text-sm">
        ...
        {withinLimit.length > 0 && (
          <>
            <Separator />
            <div className="space-y-2">
              <p className="text-muted-foreground">Within Threshold</p>
              <div className="flex flex-wrap gap-2">
